<template>
  <div style="position: relative;">
    <sticky :z-index="10" class="sub-navbar">
      <el-button icon="el-icon-plus" style="margin-left: 10px;" type="success" @click="createSpecial">
        新建专题
      </el-button>
    </sticky>
...
<script>
import { fetchAllSpecialCategory, createSpecial } from '@/api/special'
...
export default {
  ...
  methods: {
    ...
    // 点击新建按钮
    createSpecial() {
      this.$router.push('/special/create')
    },
    handleSubmit() {
      createSpecial(this.temp).then(({ message }) => {
        this.$message({
          message,
          type: 'success'
        })
