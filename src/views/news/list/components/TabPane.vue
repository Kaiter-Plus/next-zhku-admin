...
<script>
import { fetchNewsByCategory, updateNewsById, removeNewsById } from '@/api/news'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      fetchNewsByCategory(this.type, this.pageQuery).then(({ data, message }) => {
        this.list = data.item
    ...
    update(data) {
      updateNewsById(data).then(({ message }) => {
        this.$message({
          message,
          type: 'success'
        })
    ...
    handleRomove(id, index) {
      removeNewsById(id).then(({ message }) => {
