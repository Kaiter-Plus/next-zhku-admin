<template>
  <div class="app-container">
    ...
    <el-dialog title="新建新闻类型" :visible.sync="dialogFormVisible">
...
<script>
import { fetchNewsCategory, createNewsCategory, updateNewsCategory, removeNewsCategory } from '@/api/news'
...
export default {
  ...
  methods: {
    ...
    updateImage(data) {
      updateNewsCategory(data).then(({ message }) => {
        this.$message({
          message,
          type: 'success'
        })
    ...
    handleRomove(row, index) {
      removeNewsCategory(row).then(({ message }) => {
    ...
    createCategoryImage() {
      createNewsCategory(this.temp).then(({ message }) => {
        this.$message({
          message,
          type: 'success'
        })
